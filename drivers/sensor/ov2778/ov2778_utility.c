...
#define OV2778_DGAIN_H          0x315A
#define OV2778_DGAIN_L          0x315B
...
int sensor_init(sensor_info_t *sensor_info)
{
	...
	if (sensor_info->resolution == 1080) {
		vin_err("ov2778 resolution is 1080 \n");
		...
		usleep(1000);
		setting_size =
			sizeof(ov2778_init_30fps_fps_settings) / sizeof(uint32_t) / 2;
		ret = vin_write_array(sensor_info->bus_num,
					sensor_info->sensor_addr, 2,
					setting_size, ov2778_init_30fps_fps_settings);
		if (ret < 0) {
	...
	if (sensor_info->format == SENSOR_DATA_TYPE_RAW12) {
		if (sensor_info->sensor_mode == PWL_M) {
			...
			vin_err("unsupported sensor mode\n");
			return -RET_ERROR;
		}
	} else {
		vin_err("unsupported format\n");
		return -RET_ERROR;
	}
...
void ov2778_normal_data_init(sensor_info_t *sensor_info, sensor_turning_data_t *turning_data)
{
	...
	uint32_t vts = vts_hi;
	vts = vts << 8 | vts_lo;
	vin_err("vts_hi:0x%x,vts_lo:0x%x,vts:0x%x\n", vts_hi,vts_lo, vts);
	turning_data->sensor_data.lines_per_second = vts * sensor_info->fps; // TBC
	// turning_data.sensor_data.lines_per_second = 33120;   72180   // TBC
	vin_err("lines_per_second:%d\n", turning_data->sensor_data.lines_per_second);
	turning_data->sensor_data.exposure_time_max = 2163;              // TBC
	turning_data->sensor_data.exposure_time_long_max = 2 * vts - 8; // TBC
}
...
static int ov2778_linear_data_init(sensor_info_t *sensor_info)
{
	...
	sensor_data_bayer_fill(&turning_data.sensor_data, 12, (uint32_t)BAYER_RGBIR_4x4_START_RGGIR, (uint32_t)BAYER_PATTERN_GRBIR_4X4);
	sensor_data_bits_fill(&turning_data.sensor_data, 12);

	turning_data.sensor_data.gain_max = 128;            // TBC
	turning_data.sensor_data.analog_gain_max = 128; // TBC
	...
	if (turning_data.normal.again_lut != NULL) {
		memset(turning_data.normal.again_lut, 0xff, 256 * sizeof(uint32_t));
		 memcpy(turning_data.normal.again_lut, ov2778_dgain_lut,
			sizeof(ov2778_dgain_lut));
	}
...
static int sensor_aexp_gain_control(hal_control_info_t *info, uint32_t mode, uint32_t *again, uint32_t *dgain, uint32_t gain_num)
{
	...
	char again_value = 0, dgain_value_h = 0, dgain_value_l = 0;
	int ret = 0;
	...
	if (Again < 32) {
		again_value = 0x00;
		dgain_value_l = ov2778_dgain_lut[Again] & 0x00FF;
	}
	else if (Again < 64) {
		again_value = 0x01;
		dgain_value_l = ov2778_dgain_lut[Again - 32] & 0x00FF;
	}
	else if (Again < 96) {
		again_value = 0x02;
		dgain_value_l = ov2778_dgain_lut[Again - 64] & 0x00FF;
	}
	else {
		again_value = 0x03;
		dgain_value_l = ov2778_dgain_lut[Again - 96] & 0x00FF;
	}


	//dgain_value_h = (ov2778_dgain_lut[Again] >> 8) & 0x00;
	dgain_value_h = 0x01;
	//dgain_value_l = ov2778_dgain_lut[Again] & 0x00FF;

	vin_i2c_write8(bus, 16, sensor_addr, OV2778_AGAIN, again_value);
	vin_i2c_write8(info->bus_num, 16, info->sensor_addr, OV2778_DGAIN_H,dgain_value_h);
	vin_i2c_write8(info->bus_num, 16, info->sensor_addr, OV2778_DGAIN_L,dgain_value_l);
